...
public class SearchItemFrm extends JFrame implements ActionListener {
    ...
    public SearchItemFrm(User u, Appointment a) {
        ...
        JPanel pnCenter = new JPanel(new GridLayout(2, 1, 5, 5));

        // 2a. Search Results Panel (Left: Available Services, Right: Available
        // Materials)
        JPanel pnResults = new JPanel(new GridLayout(1, 2, 5, 5));
        ...
        btnSelectMaterial = new JButton("Add Material");
        ...
        // 2b. Selected Items Panel (Bottom half of Center)
        JPanel pnSelected = new JPanel(new BorderLayout());
        pnSelected.add(new JLabel("Selected Items", JLabel.CENTER), BorderLayout.NORTH);
        String[] selectedCols = { "Type", "Name", "Quantity" };
    ...
    private void updateSelectedItemsTable() {
        ...
        if (a.getAppointmentService() != null) {
            for (AppointmentService aps : a.getAppointmentService()) {
                model.addRow(new Object[] { "Service", aps.getService().getName(), aps.getQuantity() });
            }
        }
        ...
        if (a.getAppointmentMaterial() != null) {
            for (AppointmentMaterial apm : a.getAppointmentMaterial()) {
                model.addRow(new Object[] { "Material", apm.getMaterial().getName(), apm.getQuantity() });
            }
        }
    }
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource().equals(btlSearch)) {
            ...
            for (Service srv : listService) {
                modelS.addRow(new Object[] { srv.getId(), srv.getName() });
            }
            ...
            for (Material mat : listMaterial) {
                modelM.addRow(new Object[] { mat.getId(), mat.getName() });
            }
            ...
            String qtyStr = JOptionPane.showInputDialog(this, "Enter quantity for " + selectedService.getName() + ":",
                    "1");
            if (qtyStr == null)
                return; // cancelled
            ...
            String qtyStr = JOptionPane.showInputDialog(this, "Enter quantity for " + selectedMaterial.getName() + ":",
                    "1");
            if (qtyStr == null)
                return; // cancelled
            ...
            if (a.getAppointmentService() == null || a.getAppointmentService().isEmpty()) {
                JOptionPane.showMessageDialog(this,
                        "Please add at least one Service to the booking before assigning staff.");
                return;
            }
